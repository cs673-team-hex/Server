...
class BlackJackRound:
    ...
    def playerDouble(self, userid):
        if self.currentPlayer.user_id != userid:
            return False
        if self.currentPlayer.getStatus() != BlackjackPlayer.STATUS_STARTED:
            return False
    ...
    def playerHit(self, userid):
        if self.currentPlayer.user_id != userid:
            return False
        if not (self.currentPlayer.isStand or self.currentPlayer.isSurrender):
            self.currentPlayer.addCard(self.cardDeck.getTopCard())
            self.currentPlayer.isHit = True
            #  Check GameStatus
